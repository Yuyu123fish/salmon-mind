...
class WebSearchApplicationServiceTest {
    ...
    void normalizesSafeUrlsStripsHtmlAndDeduplicatesWithinProvider() {
        WebSearchProviderPort provider = new StubProvider(
        ...
        WebSearchService service = new WebSearchApplicationService(provider);

        var result = service.search(new WebSearchRequest("  问题\n", WebSearchFreshness.ANY, 5));
    ...
    void mapsProviderFailureAndInvalidInputWithoutThrowing() {
        ...
        var rateLimited = service.search(new WebSearchRequest("query", WebSearchFreshness.ANY, 5));
        var invalid = service.search(new WebSearchRequest(" ", WebSearchFreshness.ANY, 5));
    ...
    void keepsTheLowestProviderRankForNormalizedDuplicateUrls() {
        ...
        var result = service.search(new WebSearchRequest("query", WebSearchFreshness.ANY, 5));
    ...
    private record StubProvider(RawSearchResult response) implements WebSearchProviderPort {
        @Override
        public RawSearchResult search(String query, WebSearchFreshness freshness, int count) {
            return response;
        }
