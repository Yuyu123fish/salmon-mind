...
- 注释使用中文。对于关键链路的方法和类请使用规范的JavaDoc，而不是随便两个行注释，也不是罗列完整流程，那样会很臃肿，请使用简洁的语言描述它的作用。
- 不要完全省略行间注释，对于关键链路可以保留一定的行间注释，对于难以知晓其用途的代码也可以保留其行间注释。
...
- 执行Agent在执行完毕后进行汇报，同时向开发者说明关键链路的审查流程以及风险点。禁止私自删除docker容器！
- 实施需要开发者补充配置（例如 API Key、Endpoint、账号、端口或外部服务）时，实施完成后必须在交付汇报中逐项说明配置名、用途、必需性、填写位置、启动/重启要求和当前是否已实际验证；同时更新相关配置模板与稳定文档。敏感值只写占位符或环境变量名，不把真实凭据写入仓库。具体清单见 `docs/development.md` 的“开发者补充配置”。
...
- 大型 Feature 的各 Stage Plan 必须与 `spec.md` 放在同一 Feature 目录，并由该 Spec 维护可点击的 Stage Plan 索引；新增、重命名或删除 Stage Plan 时同步更新索引。
